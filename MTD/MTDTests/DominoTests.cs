...
namespace MTDTests
{
    ...
    public class DominoTests
    {
        ...
        {
            int answer = 1 + 2;
            Assert.AreEqual(3, answer); // Assert is used within nUnit. Are.Equal(expected value, output)
        }
        ...
        public void TestSettersInValidTry()
        {
            ...
            {
                d.Side1 = -1;
                Assert.Fail("The setter should throw an exception for negative values."); // if the setter does not allow this it will pass.
            }
            catch (ArgumentException)
            {
                Assert.Pass("The setter threw an exception for a negative value as expected"); // the try above will use the error thrown to pass the test.
            }
            try
            {
                d.Side1 = 15;
            ...
            Assert.AreEqual(0, d.Side1); //this is what both sides were initialized to, if anything carrys on, its the setters fault.
            Assert.AreEqual(0, d.Side2); //side did not get reset.
        }

        [Test]
        public void TestSettersInValidAssertThrows() //using lambda.
        {
            Domino d = new Domino();
